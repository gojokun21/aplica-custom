...
const { data: page } = await useAsyncData('legal-terms', () =>
  $api<LegalPage>('/legal/terms').catch(() => null),
);
useSeoMeta({ title: () => page.value?.title || 'Termeni și condiții' });
</script>

<template>
  <div class="container-page max-w-3xl py-12">
    <h1 class="text-3xl font-bold text-ink">{{ page?.title || 'Termeni și condiții' }}</h1>
