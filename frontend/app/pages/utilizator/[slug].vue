...
const { data: f, error } = await useAsyncData(`freelancer-${slug.value}`, () =>
  $api<FreelancerDetail>(`/profiles/freelancers/by-slug/${slug.value}`),
);
const { data: reviewData } = await useAsyncData(
  `freelancer-reviews-${slug.value}`,
  () =>
    f.value?.userId
      ? $api<{ items: Review[]; summary: { avg: number; count: number } }>(
          `/users/${f.value.userId}/reviews`,
        ).catch(() => ({ items: [] as Review[], summary: { avg: 0, count: 0 } }))
      : Promise.resolve({ items: [] as Review[], summary: { avg: 0, count: 0 } }),
  { watch: [() => f.value?.userId] },
);

const fullName = computed(() =>
  f.value ? `${f.value.user.firstName} ${f.value.user.lastName}` : '',
);
const rate = computed(() =>
  f.value?.hourlyRateCents != null ? `€${Math.round(f.value.hourlyRateCents / 100)}/h` : '—',
);
const plainOverview = computed(() =>
  (f.value?.overview ?? '').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 160),
);

// SEO: titlul paginii = numele persoanei + titlul profesional.
useSeoMeta({
  title: () =>
    f.value ? `${fullName.value}${f.value.title ? ' — ' + f.value.title : ''}` : undefined,
  description: () =>
    f.value
      ? plainOverview.value ||
        `${fullName.value}, freelancer pe aplica${f.value.countryCode ? ' · ' + f.value.countryCode : ''}.`
      : undefined,
  ogTitle: () => (f.value ? fullName.value : undefined),
  ogDescription: () => plainOverview.value || undefined,
  ogImage: () => f.value?.user.avatarUrl || undefined,
});
</script>

<template>
  <div class="container-page py-10">
    <NuxtLinkLocale to="/talente" class="flex items-center gap-1.5 text-sm text-body hover:text-brand-600">
    ...
    <div v-else-if="f" class="mt-6 grid gap-6 lg:grid-cols-3">
      ...
      <div class="lg:col-span-2">
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="min-w-0 flex-1">
              <div class="flex flex-wrap items-center gap-2">
                <h1 class="text-2xl font-bold text-ink">{{ fullName }}</h1>
                <span
                  v-if="f.available"
        ...
        <div v-if="reviewData && reviewData.items.length" class="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
          <h2 class="text-lg font-semibold text-ink">Recenzii ({{ reviewData.summary.count }})</h2>
